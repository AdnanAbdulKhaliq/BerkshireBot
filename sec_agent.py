...
class FinancialYearStats(BaseModel):
    ...
    metrics: Dict[str, Optional[float]] = Field(default_factory=dict, description="Dictionary of metric_key -> value")
...
class FinancialStats(BaseModel):
    years: List[FinancialYearStats] = Field(default_factory=list, description="List of financial stats by year")
    ...
    def to_markdown_table(self) -> str:
        ...
        header_cols = ["Year"] + [METRICS_CONFIG[k]["display_name"] for k in metric_keys]
...
def get_latest_filing(cik: str, form_type: str = "10-K") -> Dict:
    ...
    logger.info(f"✅ Found {form_type} filed on {filing_date}, Accession: {f.get('accessionNo')}")
    return {
...
def get_multiple_filings(cik: str, form_type: str = "10-K", count: int = 5) -> List[Dict]:
    """
    ...
    logger.info(f"✅ Found {len(results)} {form_type} filings, dates: {[r['filing_date'] for r in results]}")
    return results
...
def extract_section_by_url(filing_url_htm: str, form_type: str, section_name: str, return_type: str = "text") -> str:
...
def xbrl_metrics(accession_no: Optional[str] = None, filing_url_htm: Optional[str] = None) -> Dict:
    """
    Pull structured financials via **XbrlApi** using METRICS_CONFIG. Returns a dict with all configured metrics.
    """
    logger.info(f"💰 Extracting XBRL financial metrics (accession: {accession_no or 'None'})")
    *_, xbrlApi = _sec_clients()
        ...
        for field in xbrl_fields:
            ...
            if data:
                ...
                if period_end is None and data.get("period"):
                    period_end = data["period"].get("endDate") or data["period"].get("instant")
                break
    ...
    if result.get("revenue") and result.get("net_income") is not None and result["revenue"] != 0:
        result["profit_margin"] = result["net_income"] / result["revenue"]
    else:
        result["profit_margin"] = None
    
    logger.info(f"✅ XBRL metrics extracted: {', '.join([f'{k}: {v}' for k, v in result.items() if k not in ['period_end', 'profit_margin']])}")
    return result
...
class SecAgent:
    ...
    def __init__(self, verbose: bool = True, save_final: bool = True, save_trace: bool = True):
    ...
    def _lookup_company_cik(self, company_name: str) -> str:
        ...
        """
        if self.verbose:
            self.logger.info(f"🔧 TOOL CALLED: lookup_company_cik(company_name='{company_name}')")
        self._log_progress(f"Tool Call: lookup_company_cik\nInput: company_name='{company_name}'")
    ...
    def _fetch_latest_filings(self, input_str: str) -> str:
        ...
        """
        if self.verbose:
            self.logger.info(f"🔧 TOOL CALLED: fetch_latest_filings(input='{input_str}')")
        ...
        try:
            ...
            if self.verbose:
                self.logger.info(f"Parsed: CIK={cik}, form_type={form_type}, count={count}")
            ...
            if filings:
                ...
                if self.verbose:
                    ...
                    self.logger.info(f"🔧 TOOL RESULT: fetch_latest_filings -> Found {len(filings)} filings")
                result_str = json.dumps({"count": len(filings), "filings": filings}, indent=2)
                ...
                result_str = json.dumps({"error": f"No {form_type} filings found for CIK {cik}"})
                self._log_progress(f"Tool Result: fetch_latest_filings\nNo filings found")
                return result_str
        except Exception as e:
            if self.verbose:
    ...
    def _extract_filing_section(self, input_str: str) -> str:
        ...
        """
        if self.verbose:
            self.logger.info(f"🔧 TOOL CALLED: extract_filing_section(input='{input_str}')")
        self._log_progress(f"Tool Call: extract_filing_section\nInput: '{input_str}'")
        
        if not self.current_filings:
            if self.verbose:
                self.logger.warning("No filings cached - extract_filing_section called before fetch_latest_filings")
            error_msg = "No filings have been fetched yet. Use fetch_latest_filings first."
            self._log_progress(f"Tool Result: extract_filing_section\nError: {error_msg}")
            return error_msg
        ...
        if filing_idx >= len(self.current_filings):
            if self.verbose:
                self.logger.error(f"Filing index {filing_idx} out of range (have {len(self.current_filings)} filings)")
        ...
        if self.verbose:
            self.logger.info(f"Extracting '{section_name}' from {form_type} filed on {filing_date}")
        result = extract_section_by_url(url_htm, form_type, section_name, return_type="text")
        if self.verbose:
            self.logger.info(f"🔧 TOOL RESULT: extract_filing_section -> Extracted {len(result)} chars from filing {filing_idx}")
    ...
    def _calculate_financial_metrics(self, _: str = "") -> str:
        ...
        self._log_progress(f"Tool Call: calculate_financial_metrics\nCalculating metrics for all cached filings")
        
        if not self.current_filings:
            if self.verbose:
                self.logger.warning("No filings cached - calculate_financial_metrics called before fetch_latest_filings")
            error_msg = "No filings have been fetched yet. Use fetch_latest_filings first."
            self._log_progress(f"Tool Result: calculate_financial_metrics\nError: {error_msg}")
            return error_msg
        ...
        for idx, filing in enumerate(self.current_filings):
            ...
            filing_date = filing.get("filing_date")
            if self.verbose:
                self.logger.info(f"Calculating metrics for filing {idx} (date: {filing_date}, accession: {accession})")
        ...
        if self.verbose:
            self.logger.info(f"🔧 TOOL RESULT: calculate_financial_metrics -> Calculated metrics for {len(all_metrics)} filings")
    ...
    def _populate_year_stats(self, input_str: str) -> str:
        ...
        """
        if self.verbose:
            self.logger.info(f"🔧 TOOL CALLED: populate_year_stats(input='{input_str}')")
        ...
        try:
            ...
            if len(parts) < 2:
                ...
                error_result = json.dumps({"error": f"Invalid input format. Expected: {expected_format}"})
            ...
            if self.verbose:
                ...
                self.logger.info(f"🔧 TOOL RESULT: populate_year_stats -> Successfully added stats for {year}")
    ...
    def _create_agent(self, company_description: Optional[str] = None):
        ...
        tools = [
            self._lookup_company_cik,
            self._fetch_latest_filings,
            self._extract_filing_section,
            self._calculate_financial_metrics,
            self._populate_year_stats,
        ]

        metric_keys = list(METRICS_CONFIG.keys())
        metrics_format = "|".join([k.upper() for k in metric_keys])
        metrics_list = ", ".join([METRICS_CONFIG[k]["display_name"].lower() for k in metric_keys])
        
        company_context = f"\n\nCompany Context: {company_description}" if company_description else ""
        
        system_prompt = (
            f"""You are a financial analyst specializing in SEC filings analysis across multiple years.{company_context}
    ...
    def run(self, company: str, company_description: Optional[str] = None) -> str:
        ...
        if self.save_trace:
            self.progress_file_path = f"sec_agent_progress_{company.replace(' ', '_')}_{timestamp}.txt"
        ...
        if self.verbose:
            ...
            if self.save_trace:
                self.logger.info(f"📋 Progress will be tracked in: {self.progress_file_path}")
            self.logger.info("=" * 80)
        
        agent = self._create_agent(company_description)
        user_message = {
            "role": "user",
            "content": f"Analyze the past 5 years of SEC 10-K filings for {company} and generate a comprehensive multi-year risk and financial trends report.",
        }
        
        if self.verbose:
            self.logger.info(f"Invoking agent with message: {user_message['content']}")
        self._log_progress(f"Agent Invocation\nTask: Analyze 5 years of 10-K filings for {company}")
        ...
        try:
            ...
            if messages:
                ...
                if isinstance(content, str):
                    ...
                    report_content = "".join(part.get("text", "") if isinstance(part, dict) else str(part) for part in content)
                else:
        ...
        if self.financial_stats.years:
            if self.verbose:
                self.logger.info(f"📊 Appending financial table with {len(self.financial_stats.years)} years of data")
            financial_table = "\n\n---\n\n## Financial Metrics Summary (5-Year)\n\n" + self.financial_stats.to_markdown_table()
            report_content += financial_table
        else:
            if self.verbose:
                self.logger.warning("⚠️ No financial stats were populated during analysis")
